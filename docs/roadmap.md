...
| R24 | Transcript-only document (01, review of 630d08a): the subtitle's lifecycle field reads "Transcript Only", which a cold reader took as a constraint rather than the meeting's state | Say the state in the reader's words (the roadmap's content-reads table decides the label; candidates: "Transcript" or "Transcript, no note yet"). Not yet done |
| R25 | Rename sheet (05, review of 630d08a): the help line under the field ("Leave this empty to use the opening line from the transcript again.") restates what the placeholder already implies | Demote to caption size or drop it if the placeholder carries the rule. Not yet done |

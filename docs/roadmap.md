...
| R29 | Found while measuring R27, and it is why R27's fill was the *retired* red rather than the live one. `styles.css`'s dark block declares fourteen tokens; three of them are dead. `tokens.css` sets `--accent`, `--record` and `--attention` under `:root[data-theme="dark"]`, which outranks this file's `@media (prefers-color-scheme: dark)` + `:root`, and `main.js` sets that attribute from the system appearance. Measured live against every one: `--accent` reads #0a84ff not #8eabff, `--record` #ff453a not #ef8879, `--attention` #ffd60a not #ecc36f. The other eleven still win. So the three dead ones are exactly DESIGN.md's three reserved hues, and a reader learning the dark palette from `styles.css` gets the wrong answer for the three colors that matter most | Two consequences, neither a rendering defect today. The W9-A comment in that block justifies `--status-record-text: #ef8879` as reusing "the app's existing `--record` dark value (already proven legible … 5.40:1)"; that sentence is now false, though the measurement still holds because both it and `--record-soft` are literals this file still owns. And the app now carries two reds for one role — the Record control at #ff453a, the record status text at #ef8879. Remedy is to delete the three dead declarations and decide the status-text red against the live token, not to change tokens.css. Not yet done |
| R30 | No harness mode reaches the transcript-retry decision sheet or the delete-meeting sheet. Both are rendered by `main.js` (the retry pair at 1903, the delete pair at 2000) from state `tauri-stub.js` does not model: clicking `start-transcript-retry` in `mode=library` opens no dialog, and Manage reveals only Lock meeting. This is why neither sheet has ever appeared in a cold review, and why R27's own remedy could only be verified by computed style rather than by a frame | Add stub state for the retry decision and the delete confirmation, so both sheets are reachable the way `mode=summary-failed` made the failed note reachable. Worth doing before the next capture pass, since a sheet no fixture reaches is a sheet no review will ever judge. Not yet done |

...
| D-RECEIPT | `corpus_question::tests::the_packaged_question_receipt_describes_the_files_it_measured` has failed since 2026-09-01. `worker/embedding.py` changed in 4205c32 and `notes/packaged_question_receipt.json` was never regenerated, so the test reports the file changed since the receipt was produced. Found while running the suites for R28; unrelated to that work and to every other change today | Re-run the packaged-question probe and commit the regenerated receipt. It is a measurement, not an edit, so it needs the packaged model. Not yet done |

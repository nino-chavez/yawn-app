...
| R29 | Found while measuring R27, and it is why R27's fill was the *retired* red rather than the live one. `styles.css`'s dark block declares fourteen tokens; three of them are dead. `tokens.css` sets `--accent`, `--record` and `--attention` under `:root[data-theme="dark"]`, which outranks this file's `@media (prefers-color-scheme: dark)` + `:root`, and `main.js` sets that attribute from the system appearance. Measured live against every one: `--accent` reads #0a84ff not #8eabff, `--record` #ff453a not #ef8879, `--attention` #ffd60a not #ecc36f. The other eleven still win. So the three dead ones are exactly DESIGN.md's three reserved hues, and a reader learning the dark palette from `styles.css` gets the wrong answer for the three colors that matter most | "Cosmetic" was the wrong first answer, and measuring it found a live contrast defect (R31 below). Each `-soft` tint was picked and measured against the hue this file declares, and two of the three hues then changed underneath them. Measured every pairing that actually exists: `--attention` on `--attention-soft` improved (7.26:1 designed, 8.57:1 live); `--accent` on `--accent-soft` fell from 6.26:1 to **3.82:1**, below AA; `--record-soft` is never used as a background at all, and the W9-A comment's 5.40:1 still holds because both sides of it are literals this file still owns — though its stated reason ("reuses the app's existing `--record` dark value") is now false, and the app carries two reds for one role: the Record control at #ff453a, the record status text at #ef8879. Remedy is to delete the three dead declarations and re-decide the status-text red against the live token, not to change tokens.css. Not yet done |
...
| R34 | Latent, found while tracing the delete path: `main.js:1288` renders Move to Trash as `class="btn danger"`, and `.btn.danger` is defined in neither `styles.css` nor `tokens.css`. The class is inert, so the control currently renders as a plain `.btn`, which is what DESIGN.md wants — by accident. R20 removed the accent fill from this control and left the word behind | Either define it as the deliberate no-op it now is, or drop the word. Leaving an undefined class on a destructive control is a trap: the day someone adds a `.btn.danger` rule it applies here silently, to the one control DESIGN.md says must never be styled up. Not yet done |
